...
class ResetPasswordController extends AbstractController
{
    ...
    public function index(Request $request): Response
    {
        ...
        if ($request->get('email')) {
            ...
            if ($user) {
                ...
                // dd($url);
                $uri = $_SERVER['HTTP_HOST'] . '' . $url;

                $content = "Bonjour " .  $user->getFirstname() . "<br/>Vous avez demandé à réinitialiser votre mot de passe sur le site La Boutique Française.<br>";
                $content .= "Merci de bien vouloir cliquer sur le lien suivant pour <a href='" . $uri . "'> mettre à jour votre mot de passe </a> ";
                ...
                $this->addFlash('notice', 'Cette adresse e-mail est inconue.');
            }
        }
    ...
    public function update(Request $request, $token, UserPasswordHasherInterface $encoder): Response
    {
        ...
        if ($now > $reset_password->getCreatedAt()->modify('+ 3 hour')) {
            ...
            $this->addFlash('notice', 'Votre demande de mot de passe à éxpirée. Merci de la renouveller.');
            return $this->redirectToRoute('reset_password');
        };
